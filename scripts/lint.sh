...
echo "Running varcheck..." && varcheck $(go list ./... | grep -v /vendor/  )
echo "Running structcheck..." && structcheck $(go list ./... | grep -v /vendor/ )
# go vet is the official Go static analyzer
echo "Running go vet..." && go vet $(go list ./... | grep -v /vendor/ )
# checks for unhandled errors
echo "Running errcheck..." && errcheck $(go list ./... | grep -v /vendor/ )
# check for unnecessary conversions - ignore autogen code
echo "Running unconvert..." && unconvert -v $(go list ./... | grep -v /vendor/ )
...
echo "Running golint..." && golint -set_exit_status $(go list ./... | grep -v /vendor/ )
